...
final static float PAYLOAD_MASS = 2.0;  // kg
  ...
  void draw() {
    ...
    for(i=0;i<NUM_JOINTS-1;++i) {
      line(this.joints[i].posAbs.x*SCALE,
           BOTTOM-this.joints[i].posAbs.y*SCALE,
           this.joints[i+1].posAbs.x*SCALE,
           BOTTOM-this.joints[i+1].posAbs.y*SCALE);
    }
    ...
    if(activeJoint==-1) {
      int highlightJoint = this.findNearestJoint(new Vector(mouseX,BOTTOM-mouseY),JOINT_SELECTION_LIMIT);
      if(highlightJoint!=-1) {
        stroke(255,128,0);
        noFill();
        ellipse(this.joints[highlightJoint].posAbs.x*SCALE,
                BOTTOM-this.joints[highlightJoint].posAbs.y*SCALE,
                JOINT_DIAMETER,
                JOINT_DIAMETER);
      }
    }
    ...
    for(i=0;i<NUM_JOINTS;++i) {
      line(this.joints[i].posAbs.x*SCALE,
           ...
           BOTTOM-(this.joints[i].posAbs.y*SCALE + this.joints[i].force.y));
    }
    ...
    if(activeJoint>=0) {
      ...
      line(mouseX,mouseY,p.x,BOTTOM-p.y);
    }
    ...
    for(i=0;i<NUM_JOINTS;++i) {
      ...
      if(drawAngles) {
        ...
        text(i+"θ="+degrees(this.joints[i].angleRel),
             this.joints[i].posAbs.x*SCALE,
             BOTTOM-(this.joints[i].posAbs.y*SCALE-LINE_HEIGHT));
      }
    }
  }
